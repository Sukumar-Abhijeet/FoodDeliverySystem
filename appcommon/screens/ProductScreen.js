...
export default class ProductScreen extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      categoryName: this.props.navigation.getParam('cName'),
      dim: 150,
  ...
  async retrieveItem(key) {
    console.log("ProductScreen retrieveItem() key: ", key);
    let item = null;
    try {
  ...
  async storeItem(key, item) {
    console.log("ProductScreen storeItem() key: ", key);
    let jsonItem = null;
    try {
  ...
  async removeItem(key) {
    console.log("ProductScreen removeItem() key: ", key);
    try {
  ...
  checkKitchenStatus = () => {
    console.log("ProductScreen checkKitchenStatus()");
  ...
  getLocation = () => {
    console.log("ProductScreen getLocation()");
  ...
  initCartItems = () => {
    console.log("ProductScreen initCartItems()");
  ...
  getPincode = () => {
    console.log("ProductScreen getPincode()");
    this.retrieveItem('Address').then((data) => {
  ...
  fetchCategories = () => {
    console.log("ProductScreen fetchCategories()");
    this.retrieveItem('CategoryData').then((data) => {
      if (data != null) {
        this.setState({ modalItems: data, });
      }
      fetch(BASEPATH + Global.FETCH_ALL_CATEGORIES_URL, {
        method: "POST",
        headers: {
  ...
  fetchCategoryProducts = () => {
    console.log("ProductScreen fetchCategoryProducts() categoryId: " + this.state.categoryId, this.state.cityCode, this.state.locCode, this.state.categoryName);
    if (this.state.categoryId != null && this.state.categoryId != "") {
      fetch(BASEPATH + Global.FETCH_CATEGORY_PRODUCT_URL, {
        method: "POST",
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
        ...
        console.log("Error Category Products: ", error);
        ToastAndroid.show("We could not find an active internet connection.", ToastAndroid.SHORT);
      });
    }
    else {
  ...
  addProductToCart = (item) => {
    ...
    if (this.state.cartTypeFlag == "R") {
      ...
      this.setState({ itemCount: cartItems.length, cartItems: cartItems, visibleModal: null, selVar: {}, selAdn: [] });
  ...
  filterProductsByRating = (ratingType) => {
    console.log("ProductScreen filterProductsByRating() ratingType: ", ratingType);
  ...
  componentDidMount() {
    this.props.navigation.addListener('didFocus', () => { this.initCartItems(), this.checkKitchenStatus(); this.fetchCategoryProducts() })
    console.log("ProductScreen componentDidMount()");
    this.getLocation();
            ...
            <TouchableOpacity onPress={this.showProducts.bind(this, item.CategoryId, item.CategoryName)}>
              <View style={styles.categortItemContainer}>
                <View style={{ justifyContent: 'center', alignItems: 'center', flex: 2, }}>
                  <ImageBackground source={{ uri: BASEPATH + item.CategoryImage }} style={{ justifyContent: 'center', alignItems: 'center', backgroundColor: '#f9f9f9', width: 15, height: 15 }}>
                  </ImageBackground >
                </View>
  ...
  render() {
    ...
    return (
      <View style={styles.mainContainer}>
        ...
        <View style={styles.mainHeaderConatiner}>
          <Container style={{ flex: 1, height: null }}>
            <Header hasSegment style={{ backgroundColor: '#fff' }}>
              ...
              <Body>
                <Title style={{
                  color: '#555', marginLeft: -45
                  , paddingLeft: 0, alignSelf: 'flex-start'
                }}>{this.props.navigation.getParam('cName')}</Title>
              </Body>
            </Header>
            <Segment>
        ...
        <Modal isVisible={this.state.visibleModal === 5} style={styles.bottomModal} onBackButtonPress={() => { this.setState({ visibleModal: null }) }}>
          {this._renderModalContent()}
        </Modal>

        <Modal isVisible={this.state.visibleModal === 7 && this.state.customisationModal == 7} style={styles.bottomModal} onBackButtonPress={() => this.setState({ visibleModal: null, selVar: {}, selAdn: [], customisationModal: null })}>
              ...
              renderItem={item => (
                <View style={styles.itemContainer}>
                  ...
                  <Display enable={item.ImageAvailable} style={{
                    ...
                    <ImageBackground source={{ uri: BASEPATH + item.ProductImage }} style={[styles.productImage, { opacity: (!item.Addable ? .6 : 1) }]}>
                      <View style={{ flexDirection: 'row' }}>
                        <View style={{ flex: 1 }}>
